/* file: wfdblib.h	G. Moody	13 April 1989
                        Last revised:	 1 March 2010       wfdblib 10.5.0
...
/* DEFWFDB is the default value of the WFDB path if the WFDB environment
   ...
   If WFDB_NETFILES support is disabled, the string ". /opt/wfdb-10.5.10/build/database" is
   usually sufficient for a default WFDB path, thus restricting the search for
   WFDB files to the current directory ("."), followed by /opt/wfdb-10.5.10/build/database).
...
# define DEFWFDB	". /opt/wfdb-10.5.10/build/database"
#else
# define DEFWFDB ". /opt/wfdb-10.5.10/build/database http://physionet.org/physiobank/database"
#endif
...
#include <src/Utilities/wfdb64/include/curl/curl.h>
#else
# include <WWWLib.h>
# include <WWWInit.h>
#endif
